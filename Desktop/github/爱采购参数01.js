...
function srtlistrunction() {
    errorpercentage = (badbrandvalue + badparametervalue) / (brandvalue + parameterlvaue)
    strlist = query + '\t' + badbrandvalue + '\t' + brandvalue + '\t' + badbranddemo + '\t' + badparametervalue + '\t' + parameterlvaue + '\t' + errorpercentage + '\t' + badparameterdemo
    copyHandle(strlist);
}
...
function clickclick(sumbutton9, xxx) {
    ...
    if (sumbutton9.style['text-decoration'] == "line-through") {
        ...
        if (xxx == 0) {
            ...
            badparametervalue = badparametervalue + 1
            badparameterdemo = badparameterdemo + sumbutton9.innerText + ";"
        }
        console.log("品牌错误:" + badbrandvalue)
        console.log("参数错误:" + badparametervalue)
    }
